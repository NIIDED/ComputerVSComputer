...
public class PVP_NinjaVer2 : MonoBehaviour
{
    ...
    public GameObject fireBallUpPrefab;
    ...
    void Update(){
        ...
        if(EnemyTrns!= null){           //敵が見つかったとき          
            ...
            if(Mathf.Abs(this.transform.position.x - EnemyTrns.position.x ) < lengthToJumpLast){            //忍者攻撃がジャンプしながら
                if(IsGround() && JumpRandom()){
                //if(IsGround()){
                    Jump();                
                }
            }
...
*/
    ...
    void Action1(){
                ...
                FireBall();     //索敵範囲に入ってないと攻撃できない（そもそもactionが呼び出されない）
    }
    ...
    void FireBall(){
        if(chargeTime < time ){ 
            if(this.transform.position.x - EnemyTrns.position.x > 2.0f){
                    Instantiate(fireBallLeftPrefab,attackPoint.transform.position,Quaternion.identity);   
                    time = 0f;
            }
            if(Mathf.Abs(this.transform.position.x - EnemyTrns.position.x) < 2.0f){
                    Instantiate(fireBallUpPrefab,attackPoint.transform.position,Quaternion.Euler(0, 0, 90f));   
                    time = 0f;
            }
            if(this.transform.position.x - EnemyTrns.position.x < -2.0f){
                    Instantiate(fireBallRightPrefab,attackPoint.transform.position,Quaternion.identity);   
                    time = 0f;
            }
        }
    }
