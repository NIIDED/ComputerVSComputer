...
public class PVP_Ninja_BodyVer2 : MonoBehaviour
{
    Rigidbody2D rb;

    PVP_NinjaVer2 PVP_PlayNinja;
    ...
    void Start(){
        PVP_PlayNinja = GetComponentInParent<PVP_NinjaVer2>();        //親のスクリプト（コンポーネント）を得るにはGetComponentParent<>()やって、そのコンポーネント名.関数でいける
    ...
    void OnTriggerEnter2D(Collider2D collision){
        ...
        PVP_Fox_BodyVer2 enemy = collision.GetComponent<PVP_Fox_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            ...
            {        
                PVP_PlayFox.GetComponent<PVP_FoxVer2>().AttackedByEnemy(PVP_PlayNinja.attack);　//敵にアタック
        ...
        PVP_Frog_BodyVer2 enemy = collision.GetComponent<PVP_Frog_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayFrog.GetComponent<PVP_FrogVer2>().AttackedByEnemy(PVP_PlayNinja.attack);　//敵にアタック
                PVP_PlayNinja.Jump();   //相手を踏んで自分がジャンプ
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
        ...
        PVP_Eagle_BodyVer2 enemy = collision.GetComponent<PVP_Eagle_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayEagle.GetComponent<PVP_EagleVer2>().AttackedByEnemy(PVP_PlayNinja.attack);　//敵にアタック
                PVP_PlayNinja.Jump();   //相手を踏んで自分がジャンプ
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
