...
public class PVP_Eagle_BodyVer2 : MonoBehaviour
{
    PVP_EagleVer2 PVP_PlayEagle;
    ...
    void Start(){
        PVP_PlayEagle = GetComponentInParent<PVP_EagleVer2>();        //親のスクリプト（コンポーネント）を得るにはGetComponentParent<>()やって、そのコンポーネント名.関数でいける
    ...
    void OnTriggerEnter2D(Collider2D collision){ 
        ...
        PVP_Fox_BodyVer2 enemy = collision.GetComponent<PVP_Fox_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y ) //自分が相手より高いときに
            {        
                PVP_PlayFox.GetComponent<PVP_FoxVer2>().AttackedByEnemy(PVP_PlayEagle.attack);　//敵にアタック
        ...
        PVP_Frog_BodyVer2 enemy = collision.GetComponent<PVP_Frog_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayFrog.GetComponent<PVP_FrogVer2>().AttackedByEnemy(PVP_PlayEagle.attack);　//敵にアタック
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
                Debug.Log("(3) プレイヤー１がダメージを食らった！！");
        ...
        PVP_Ninja_BodyVer2 enemy = collision.GetComponent<PVP_Ninja_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayNinja.GetComponent<PVP_NinjaVer2>().AttackedByEnemy(PVP_PlayEagle.attack);　//敵にアタック
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
