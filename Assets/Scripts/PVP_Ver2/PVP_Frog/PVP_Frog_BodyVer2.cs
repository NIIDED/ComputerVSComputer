...
public class PVP_Frog_BodyVer2 : MonoBehaviour
{
    PVP_FrogVer2 PVP_PlayFrog;
    ...
    void Start(){
        PVP_PlayFrog = GetComponentInParent<PVP_FrogVer2>();  //親のスクリプト（コンポーネント）を得るにはGetComponentParent<>()やって、そのコンポーネント名.関数でいける
    ...
    void OnTriggerEnter2D(Collider2D collision){
        ...
        PVP_Fox_BodyVer2 enemy = collision.GetComponent<PVP_Fox_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            ...
            {
                PVP_PlayFox.GetComponent<PVP_FoxVer2>().AttackedByEnemy(PVP_PlayFrog.attack);
                PVP_PlayFrog.Jump();
        ...
        PVP_Ninja_BodyVer2 enemy = collision.GetComponent<PVP_Ninja_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  > enemy.transform.position.y )
            {
                PVP_PlayNinja.GetComponent<PVP_NinjaVer2>().AttackedByEnemy(PVP_PlayFrog.attack);
                PVP_PlayFrog.Jump();
        ...
        PVP_Eagle_BodyVer2 enemy = collision.GetComponent<PVP_Eagle_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  > enemy.transform.position.y )
            {
                PVP_PlayEagle.GetComponent<PVP_EagleVer2>().AttackedByEnemy(PVP_PlayFrog.attack);
                PVP_PlayFrog.Jump();
