...
public class PVP_Fox_BodyVer2 : MonoBehaviour
{
    PVP_FoxVer2 PVP_PlayFox;
    ...
    void Start(){
        PVP_PlayFox = GetComponentInParent<PVP_FoxVer2>();        //親のスクリプト（コンポーネント）を得るにはGetComponentParent<>()やって、そのコンポーネント名.関数でいける
    ...
    void OnTriggerEnter2D(Collider2D collision){
        ...
        PVP_Frog_BodyVer2 enemy = collision.GetComponent<PVP_Frog_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            ...
            {        
                PVP_PlayFrog.GetComponent<PVP_FrogVer2>().AttackedByEnemy(PVP_PlayFox.attack);　//敵にアタック
        ...
        PVP_Ninja_BodyVer2 enemy = collision.GetComponent<PVP_Ninja_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayNinja.GetComponent<PVP_NinjaVer2>().AttackedByEnemy(PVP_PlayFox.attack);　//敵にアタック
                PVP_PlayFox.Jump();   //相手を踏んで自分がジャンプ
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
        ...
        PVP_Eagle_BodyVer2 enemy = collision.GetComponent<PVP_Eagle_BodyVer2>(); //攻撃する相手のスクリプトいじった時はここも変える！！
            if(this.transform.position.y  >  enemy.transform.position.y )
            {        
                PVP_PlayEagle.GetComponent<PVP_EagleVer2>().AttackedByEnemy(PVP_PlayFox.attack);　//敵にアタック
                PVP_PlayFox.Jump();   //相手を踏んで自分がジャンプ
            }//０.３離れていないとお互いダメージ喰らう else にも含まれない
            else    
            {
