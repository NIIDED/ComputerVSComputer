...
{
    // WhatCommandToInput() の改善。ちょっと近いとジャンプして、その中でさらに近くなると横へ力入れて踏むように
    ...
    float WhatCommandToInput(){
        ...
        if(Mathf.Abs(this.transform.position.x - enemy.transform.position.x) < 0.6f){
            return input = RightOrLeft();
        }
        else if(Mathf.Abs(this.transform.position.x - enemy.transform.position.x) < 2.0f){
            return input = 5.0f;
        }
        else{
            return input = RightOrLeft();
