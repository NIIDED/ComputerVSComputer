...
public class PVP_PlayerAI : MonoBehaviour
{
    //PVP プレイヤー１（ねずみ）のコントローラ
    //HP 実装
    //攻撃実装
    ...
    float WhatCommandToInput(){
        if(Mathf.Abs(this.transform.position.x - EnemyTrns.position.x ) < lengthToStartThink){
                if(!IsGround()){                //1.5以内で自分が空中にいるなら敵の方へ空気中移動する
                    ...
                    Debug.Log("(1) PLAYER2がジャンプしていた！！");        //下の"相手がジャンプしないけど、とりあえず近寄る！"で近寄って敵がジャンプしたときにジャンプするともう手遅れ
                    return input = 5.0f;
    ...
    public void GetEnemyInsideSearch(){
        Collider2D[] searchedEnemys = Physics2D.OverlapCircleAll(SearchPoint.position,searchRadius,enemy2Layer); //レイヤー変えないと自分自分もスキャン対象
            ...
            if(searchedEnemys.Length > 0){
                ...
                foreach(Collider2D searchedEnemy in searchedEnemys){
                   Vector2 posVec = searchedEnemy.transform.position- this.transform.position;
                    float angle = Vector2.Angle(transform.right* direction, posVec);    //angle は索敵範囲の半角。つまり視野角９０どがいいならangleが４５以内でないといけないので
                    if(searchAngle <= angle){   //searchedEnemyは実際の視野角の半分を入力
                        EnemyTrns = null;
    ...
    public void AttackedByEnemy(float at){
        hp -= at;
        gameManager.ChangePlayer1HpText(hp);
        Debug.Log("(1) HPが"+hp+"になった");
        rb.AddForce(Vector2.left*direction*atPower);
        //攻撃喰らうと後ろに相手を下げさせるコマンド欲しい
        if(hp <= 0){
            //一定ダメージ超えると破壊
            Destroy(this.gameObject);
            Debug.Log("(2) PLAYER1 を倒したぞ！！");    //相手に攻撃れて自分のHP減ったり倒されたりする
